...
func serializeMsgRestore(s *SPlaying, player *PlayerHolder) *pokerface.MsgRestore {
	...
	landlordChairID := int32(4)

	landlordPlayer := s.room.landlordPlayer()
	if landlordPlayer != nil {
		landlordChairID = int32(landlordPlayer.chairID)
	}
	msgRestore.LandlordChairID = &landlordChairID

	multiple32 := int32(s.room.markup)
	msgRestore.Multiple = &multiple32

	msgRestore.LastAwardCards = s.lastAwardCards

	// 下发那些加注的玩家座位ID
	callDoublePlayers := make([]int32, 0, len(s.players))
	for _, p := range s.players {

		callDoublePlayers = append(callDoublePlayers, int32(p.chairID))
		if (p.expectedAction & int(ActionType_enumActionType_CallDouble)) != 0 {
			callDoublePlayers = append(callDoublePlayers, int32(1))
		} else {
			callDoublePlayers = append(callDoublePlayers, int32(0))
		}
	...
	msgRestore.PlayersListA = callDoublePlayers
...
func serializeMsgActionResultNotifyForDraw(player *PlayerHolder, cards []*Card) *pokerface.MsgActionResultNotify {
	var msg = &pokerface.MsgActionResultNotify{}
	var action32 = int32(ActionType_enumActionType_DRAW)
	msg.Action = &action32
	var cardCountInWall32 = int32(0)
	msg.CardsInWall = &cardCountInWall32
	var chairID32 = int32(player.chairID)
	msg.TargetChairID = &chairID32

	if len(cards) > 0 {
		flowerIDs := make([]int32, len(cards))
		for i, c := range cards {
			flowerIDs[i] = int32(c.cardID)
		}

		hand := &pokerface.MsgCardHand{}
		hand.Cards = flowerIDs
		cardHandTypeNone := int32(0)
		hand.CardHandType = &cardHandTypeNone
		msg.ActionHand = hand
	}
...
func serializeMsgHandOver(s *SPlaying, handOverType int) *pokerface.MsgHandOver {
	...
	for _, player := range s.players {
		...
		var specialScore32 = int32(s.room.markup)
		msgPlayerScore.SpecialScore = &specialScore32 // 房间倍数

		var fakeWinScoreFlag32 = int32(player.cards.bombOrRocketCountOfDiscarded())
		msgPlayerScore.FakeWinScore = &fakeWinScoreFlag32 // 炸弹个数

		var continuousBankerMultiple = int32(0)
		msgPlayerScore.ContinuousBankerMultiple = &continuousBankerMultiple

		var isContinuosBanker = sc.spring
		msgPlayerScore.IsContinuousBanker = &isContinuosBanker // 是否春天

		if handOverType != int(HandOverType_enumHandOverType_None) {
			greatWin := &pokerface.MsgPlayerScoreGreatWin{}
			var baseWinScore32 = int32(s.room.config.baseScore) // 基本分数
			...
			greatWin.ContinuousBankerExtra = &endXScore
