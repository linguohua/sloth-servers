...
// landlordPlayer 当前的地主，注意如果牌局没有过了叫地主阶段，可能返回nil
func (r *Room) landlordPlayer() *PlayerHolder {
	for _, p := range r.players {
		if p.userID() == r.landlordUserID {
			return p
		}
	}
...
func (r *Room) writePlayersStatis() {
	...
	for _, p := range r.players {
		...
		dailyStatisTable := fmt.Sprintf(gconst.GameServerDailyStatisTablePrefix, dd, userID, gconst.RoomType_DDZ)
		if p.sctx != nil {
...
func (r *Room) is2PlayerRoom() bool {
	return r.config.playerNumAcquired == 2
}

func (r *Room) winAbleRemainCount(player *PlayerHolder) int {
	if !r.is2PlayerRoom() {
		return 0
	}

	if r.config.noPriority4Farmer {
		return 0
	}

	if player.userID() == r.landlordUserID {
		return 0
	}

	// 只有农民有优惠，这里大规模玩家时可能有性能隐患
	return int(math.Log2(float64(r.markup))) + 1
}
