...
func (tm *CardMgr) cardsDiscardAble(player *PlayerHolder, cards []int32) (*pokerface.MsgCardHand, bool) {
	if player.hStatis.isFirstDiscarded && player.hStatis.actionCounter == 0 {
		// 第一次出牌，必须要包含红桃3
		found := false
		for _, c := range cards {
			if c == int32(R3H) {
		...
		if !found {
			log.Println("player is first one to discard, need R3H, but found none, chair:", player.chairID)
			return nil, false
		}
	}
