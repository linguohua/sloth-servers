...
func (mgr *MonkeyMgr) attachDealCfg2Room(w http.ResponseWriter, r *http.Request, body string) {
	...
	if cfg.isForceConsistent {
		for i, tc := range cfg.monkeyUserTilesCfgList {
			if tc.userID == "" {
				w.WriteHeader(404)
	...
	case mahjong.RoomState_SRoomWaiting:
		...
		if cfg.isForceConsistent {
			for i, p := range room.players {
				if p.userID() != cfg.monkeyUserTilesCfgList[i].userID {
					w.WriteHeader(404)
	...
	if !err {
		...
		if cfg.monkeyUserTilesCfgList[0].userID != "" {
			room.bankerUserID = cfg.monkeyUserTilesCfgList[0].userID
		}
...
func (mgr *MonkeyMgr) onUploadCfgs(body string) ([]*MonkeyCfg, error) {
	...
	for {
		...
		// 4 个玩家的手牌，花牌，和动作提示
		for i := 0; i < 4; i++ {
			tuc := mgr.extractUserTilesCfg(record, i)
			if tuc != nil {
				cfg.monkeyUserTilesCfgList = append(cfg.monkeyUserTilesCfgList, tuc)
			}
		}
...
// loadTableConfigIfExist 从程序目录下加载monkey房间的配置文件
func loadTableConfigIfExist() *RoomConfig {
	...
	// var filePath = Path.Combine(dir, @"mt.json");
	var fullpath = path.Join(dir, "mt.json")

	var cfg = &RoomConfigJSON{}

	f, err := os.Open(fullpath)
	if err != nil {
		log.Println("failed to open monkey room config file:", fullpath)
		return nil
	}
	...
	log.Println("load monkey room config OK:", cfg)
	return roomConfig
}
