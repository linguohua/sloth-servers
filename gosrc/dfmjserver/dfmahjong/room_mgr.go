...
func (rm *RoomMgr) onDeleteRoomReq(msgBag *gconst.SSMsgBag) {
	...
	room.deleteReason = mahjong.RoomDeleteReason_DisbandByOwnerFromRMS
...
func (rm *RoomMgr) onUpdatePropsCfg(msgBag *gconst.SSMsgBag) {
	...
	msg := &mahjong.MsgUpdatePropCfg{}
	msg.PropCfg = &cfgString
	buf := formatGameMsg(msg, int32(mahjong.MessageCode_OPUpdatePropCfg))
	...
	for {
		...
		for _, r := range roomMgr.rooms {
			...
			if diff > 6*time.Hour {
				...
				// 执行房间解散流程
				roomMgr.forceDisbandRoom(r, mahjong.RoomDeleteReason_IdleTimeout)
			} else {
				// 处理黑名单超时
				if r.rbl != nil {
...
func (rm *RoomMgr) forceDisbandRoom(r *Room, reason mahjong.RoomDeleteReason) {
