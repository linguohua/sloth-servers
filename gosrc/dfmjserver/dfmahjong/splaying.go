...
func (s *SPlaying) chooseBanker(washout bool) {
	...
	// 流局，庄家继续做庄
	if washout {
		if s.room.config.isDoubleScoreWhenContinuousBanker {
			// 家家庄生效
			s.room.markup = 1
			newBanker = curBanker
		} else {
		...
		s.room.markup = 0
		// 庄家赢牌，继续做庄
		if curBanker.sctx.isWin() {
			if s.room.config.isDoubleScoreWhenContinuousBanker {
				curBanker.gStatis.isContinuousBanker = true
			}
			newBanker = curBanker
		} else {
			newBanker = s.tileMgr.rightOpponent(curBanker)
	...
	if action == int(mahjong.ActionType_enumActionType_DISCARD) {
		...
		if curPlayer == s.room.bankerPlayer() && curPlayer.hStatis.actionCounter == 1 {
			if xflags == 1 && curPlayer.tiles.readyHandAble() {
				...
				if curPlayer.hStatis.isWinAbleLocked {
					// 天胡下起手听，也相当于错过了一次胡牌机会
					curPlayer.hStatis.isRichiWinableOnlySelfDrawn = true
				}
