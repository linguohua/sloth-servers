...
func (s *SWaiting) getStateConst() mahjong.RoomState {
	return mahjong.RoomState_SRoomWaiting
}
...
func (s *SWaiting) onMessage(iu IUser, gmsg *mahjong.GameMessage) {
	...
	op := gmsg.GetOps()
	switch op {
	case int32(mahjong.MessageCode_OPPlayerReady):
		s.room.cl.Println("got player ready:", player.chairID)
		if player.state == mahjong.PlayerState_PSReady {
			// 重复收到ready消息
			break
		}

		player.state = mahjong.PlayerState_PSReady
		...
		for _, p := range s.room.players {
			if p.state != mahjong.PlayerState_PSReady {
				allReady = false
				break
			}
		}
		...
		if s == s.room.state {
			s.room.state2(s, mahjong.RoomState_SRoomPlaying)
		}
		break
	case int32(mahjong.MessageCode_OPKickout):
		// 踢人
		if player.userID() != s.room.ownerID {
			sendKickoutError(player, mahjong.KickoutResult_KickoutResult_FailedNeedOwner)
			return
		}
		...
		if victim == nil {
			sendKickoutError(player, mahjong.KickoutResult_KickoutResult_FailedPlayerNotExist)
			return
		}
...
func (s *SWaiting) doKickoutPlayer(player *PlayerHolder, victim *PlayerHolder) {
	msg := &mahjong.MsgKickoutResult{}
	var result32 = int32(mahjong.KickoutResult_KickoutResult_Success)
	msg.Result = &result32
	...
	for _, p := range s.room.players {
		p.sendMsg(msg, int32(mahjong.MessageCode_OPKickout))
	}
...
func sendKickoutError(player *PlayerHolder, result mahjong.KickoutResult) {
	msg := &mahjong.MsgKickoutResult{}
	var status32 = int32(result)
	msg.Result = &status32

	player.sendMsg(msg, int32(mahjong.MessageCode_OPKickout))
}
...
func (s *SWaiting) onPlayerEnter(player *PlayerHolder) {
	...
	room := s.room
	player.state = mahjong.PlayerState_PSNone
...
func (s *SWaiting) onPlayerLeave(player *PlayerHolder) {
	...
	if len(room.players) < 1 {
		// 如果玩家都离开了，则回到空闲状态
		room.state2(s, mahjong.RoomState_SRoomIdle)
	}
