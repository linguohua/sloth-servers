...
func serializeMsgHandOver(s *SPlaying, handOverType int) *mahjong.MsgHandOver {
	...
	for _, player := range s.players {
		...
		var specialScore32 = int32(sc.specialScore)
		...
		var continuousBankerMultiple32 = int32(sc.fContinuousBankerMultiple * 10) // 如果是连庄，此处带连庄倍数，如果是家家庄，此处带家家庄倍数
		msgPlayerScore.ContinuousBankerMultiple = &continuousBankerMultiple32

		if sc.isRichiPay1P {
			fakeLikeArr := []int32{int32(1)}
			msgPlayerScore.FakeList = fakeLikeArr
		}
		...
		if sc.greatWinType != 0 {
			...
			var trimGreatWinPoints32 = int32(sc.fTrimGreatWinPoints * 10)
			greatWin.TrimGreatWinPoints = &trimGreatWinPoints32

			msgPlayerScore.GreatWin = greatWin
		} else {
			miniWin := &mahjong.MsgPlayerScoreMiniWin{}
			var miniWinType32 = int32(sc.miniWinType)
			miniWin.MiniWinType = &miniWinType32
			var miniWinBasicScore32 = int32(roundFloat32(sc.fMiniWinBasicScore * 10))
			miniWin.MiniWinBasicScore = &miniWinBasicScore32
			var miniWinFlowerScore32 = int32(roundFloat32(sc.fMiniWinFlowerScore))
			miniWin.MiniWinFlowerScore = &miniWinFlowerScore32
			var miniMultiple32 = int32(roundFloat32(sc.fMiniMultiple * 10))
			miniWin.MiniMultiple = &miniMultiple32
			var miniWinTrimScore32 = int32(0)
			miniWin.MiniWinTrimScore = &miniWinTrimScore32
			// var continuousBankerExtra32 = int32(sc.continuousBankerExtra)
			// miniWin.ContinuousBankerExtra = &continuousBankerExtra32

			msgPlayerScore.MiniWin = miniWin
		}

		// if sc.fakers != nil && len(sc.fakers) > 0 {
		// 	msgPlayerScore.FakeList = make([]int32, len(sc.fakers))
		// 	for i, f := range sc.fakers {
		// 		msgPlayerScore.FakeList[i] = int32(f.chairID)
		// 	}
		// }
