...
type Room struct {
	...
	clubID     string // 如果房间是由俱乐部创建，则clubID不为空，ownerID填的是俱乐部部长，或者某个成员
	...
	scoreRecords []*mahjong.MsgRoomHandScoreRecord

	deleteReason mahjong.RoomDeleteReason
	...
	msgReplayRoom *mahjong.MsgReplayRoom
...
func newBaseRoom(ownerID string, clubID string, ID string, roomNumber string) *Room {
	...
	r.scoreRecords = make([]*mahjong.MsgRoomHandScoreRecord, 0, 16)
	...
	// 进入空闲状态
	r.state2(nil, mahjong.RoomState_SRoomIdle)
...
func newRoomFromMgr(ownerID string, clubID string, ID string, configID string,
	configJSON *RoomConfigJSON, roomNumber string) *Room {
	r := newBaseRoom(ownerID, clubID, ID, roomNumber)
	r.isForMonkey = false
...
// onHandOver 一手牌结局
func (r *Room) onHandOver(msgHandOver *mahjong.MsgHandOver) {
	...
	if r.handRoundFinished == r.config.handNum {
		...
		roomMgr.forceDisbandRoom(r, mahjong.RoomDeleteReason_DisbandMaxHand)
		return
	}

	if r.isInLoseProtectedState() {
		// 已经有足够的人进入保护状态，解散房间
		r.cl.Println("there are enough player fall into protect state, need to disband room")
		r.onGameOver(msgHandOver)

		roomMgr.forceDisbandRoom(r, mahjong.RoomDeleteReason_DisbandInLoseProtected)
		return
	}
	...
	r.state2(r.state, mahjong.RoomState_SRoomWaiting)
...
func (r *Room) onGameOver(msgHandOver *mahjong.MsgHandOver) {
	...
	for _, p := range r.players {
		...
		p.sendMsg(msg, int32(mahjong.MessageCode_OPGameOver))
	}
}
...
func (r *Room) onUserMessage(user IUser, msg []byte) {
	...
	if err != nil {
		r.cl.Printf("onUserMessage, userID:%s, error%v:\n", user.userID(), err)
		return
	}
	...
	var handled = false
	switch msgCode {
	case int32(mahjong.MessageCode_OPPlayerLeaveRoom):
		player := r.getPlayerByUserID(user.userID())
		player.allowedLeave = r.allow2Leave(player)

		if !player.allowedLeave {
			msg2 := &mahjong.MsgEnterRoomResult{}
			var status32 = int32(1)
			msg2.Status = &status32
			buf := formatGameMsg(msg2, int32(mahjong.MessageCode_OPPlayerLeaveRoom))
		...
		// 发送结果给客户端
		buf := formatGameMsg(nil, int32(mahjong.MessageCode_OPPlayerLeaveRoom))
		user.send(buf)
		// 断开websocket连接
		user.closeWebsocket()
		handled = true
		break
	case int32(mahjong.MessageCode_OPDisbandRequest):
		r.onDisbandRequest(user, gmsg)
		handled = true
		break
	case int32(mahjong.MessageCode_OPDisbandAnswer):
		r.onDisbandAnswer(user, gmsg)
		handled = true
		break
	case int32(mahjong.MessageCode_OPKickout):
		if r.handRoundStarted > 0 {
			player := r.getPlayerByUserID(user.userID())
			sendKickoutError(player, mahjong.KickoutResult_KickoutResult_FailedGameHasStartted)
			handled = true
		}
		break
	case int32(mahjong.MessageCode_OPDonate):
		r.onDonateRequest(user, gmsg)
		handled = true
		break
	case int32(mahjong.MessageCode_OP2Lobby):
		r.onUser2Lobby(user, gmsg)
		handled = true
		break
	case int32(mahjong.MessageCode_OPPing):
		xd := gmsg.GetData()
		// bits := binary.LittleEndian.Uint64(xd)
		// float := math.Float64frombits(bits)
		buf := formatGameMsgByData(xd, int32(mahjong.MessageCode_OPPong))
		user.send(buf)

		// log.Println("got ping message! send back data:", float)
		handled = true
		break
	case int32(mahjong.MessageCode_OPPong):
		handled = true
		break
	default:
		break
...
func (r *Room) onUser2Lobby(user IUser, gmsg *mahjong.GameMessage) {
	...
	if r.handRoundStarted > 0 {
		...
		var status32 = int32(1)
		msg2.Status = &status32
		buf := formatGameMsg(msg2, int32(mahjong.MessageCode_OP2Lobby))
	...
	// 发送结果给客户端
	buf := formatGameMsg(nil, int32(mahjong.MessageCode_OP2Lobby))
	user.send(buf)
	// 断开websocket连接
	user.closeWebsocket()
}
...
func (r *Room) reset() {
	...
	if r.state.getStateConst() == mahjong.RoomState_SRoomIdle {
		return
	}

	// 首先转到空闲状态以便在玩家断线时删除玩家
	if r.state.getStateConst() == mahjong.RoomState_SRoomPlaying {
		r.state2(r.state, mahjong.RoomState_SRoomWaiting)
	}
	...
	for _, p := range r.players {
		if p.state == mahjong.PlayerState_PSOffline {
...
// destroy 房间删除处理
func (r *Room) destroy() {
	reasonStr, ok := mahjong.RoomDeleteReason_name[int32(r.deleteReason)]
	if ok {
		r.cl.Printf("room now destroy, reason:%s\n", reasonStr)
	} else {
		r.cl.Printf("room now destroy, reason:%d\n", r.deleteReason)
	}

	// 通知所有人房间已经被删除
	msgDelete := &mahjong.MsgRoomDelete{}
	var reason32 = int32(r.deleteReason)
	msgDelete.Reason = &reason32

	for _, p := range r.players {
		p.sendMsg(msgDelete, int32(mahjong.MessageCode_OPRoomDeleted))
	}

	// 强制停止游戏，状态转换到deleted状态
	r.state2(r.state, mahjong.RoomState_SRoomDeleted)
...
func (r *Room) onUserOffline(user IUser, lock bool) {
	r.cl.Println("onUserOffline, userID:", user.userID())
...
func (r *Room) userTryEnter(ws *websocket.Conn, userID string) IUser {
	...
	if r.isForceConsistent() && player == nil {
		monkeyUserTileCfg := r.monkeyCfg.getMonkeyUserTilesCfg(userID)
		if monkeyUserTileCfg == nil {
			sendEnterRoomError(ws, userID, mahjong.EnterRoomStatus_MonkeyRoomUserIDNotMatch)
			return nil
		}

		// 而且玩家进入的顺序必须严格按照配置指定
		loginSeq := len(r.players)
		if loginSeq != monkeyUserTileCfg.index {
			sendEnterRoomError(ws, userID, mahjong.EnterRoomStatus_MonkeyRoomUserLoginSeqNotMatch)
			return nil
		}
	}
	...
	if len(r.players) == r.config.playerNumAcquired {
		// 已经满员
		sendEnterRoomError(ws, userID, mahjong.EnterRoomStatus_RoomIsFulled)
		return nil
	}

	// 只有空闲状态，或者等待状态才允许玩家进入
	status := r.state.getStateConst()
	if status != mahjong.RoomState_SRoomIdle && status != mahjong.RoomState_SRoomWaiting {
		sendEnterRoomError(ws, userID, mahjong.EnterRoomStatus_RoomPlaying)
		return nil
	}

	// 检查是否在黑名单里面
	if r.rbl != nil && r.rbl.has(userID) {
		sendEnterRoomError(ws, userID, mahjong.EnterRoomStatus_InRoomBlackList)
		return nil
	}

	// 房间正在解散状态，不允许进入
	if r.disband != nil {
		sendEnterRoomError(ws, userID, mahjong.EnterRoomStatus_RoomInApplicateDisband)
		return nil
	}
	...
	// 发送成功进入房间通知给客户端
	sendEnterRoomError(ws, userID, mahjong.EnterRoomStatus_Success)
...
func (r *Room) writeOnlinePlayerList2Redis(conn redis.Conn) {
	...
	if err != nil {
		r.cl.Println("writeOnlinePlayerList2Redis marshal error:", err)
	} else {
...
func (r *Room) onPlayerReconnect(player *PlayerHolder, ws *websocket.Conn) IUser {
	r.cl.Printf("onPlayerReconnect, userID:%s\n", player.userID())
	user := player.user
	...
	// 发送成功进入房间通知给客户端
	sendEnterRoomError(ws, player.userID(), mahjong.EnterRoomStatus_Success)
...
// state2 房间切换状态
func (r *Room) state2(oldState IState, newStateCode mahjong.RoomState) {
	...
	var newState IState
	switch newStateCode {
	case mahjong.RoomState_SRoomIdle:
		newState = &SIdle{room: r}
		break
	case mahjong.RoomState_SRoomDeleted:
		newState = &SDeleted{room: r}
		break
	case mahjong.RoomState_SRoomWaiting:
		newState = &SWaiting{room: r}
		break
	case mahjong.RoomState_SRoomPlaying:
...
func (r *Room) updateRoomInfo2All() {
	if len(r.players) > 0 {
		...
		for _, p := range r.players {
			p.sendMsg(msgRoomInfo, int32(mahjong.MessageCode_OPRoomUpdate))
		}
	}
...
func sendEnterRoomError(ws *websocket.Conn, userID string, status mahjong.EnterRoomStatus) {
	if status != mahjong.EnterRoomStatus_Success {
		statusStr, ok := mahjong.EnterRoomStatus_name[int32(status)]
		if ok {
	...
	msg := &mahjong.MsgEnterRoomResult{}
	var status32 = int32(status)
	msg.Status = &status32

	buf := formatGameMsg(msg, int32(mahjong.MessageCode_OPPlayerEnterRoom))
...
func (r *Room) onDonateRequest(user IUser, gmsg *mahjong.GameMessage) {
	var msgDonate = &mahjong.MsgDonate{}
	...
	if status == int32(gconst.SSMsgError_ErrTakeoffDiamondFailedNotEnough) {
		player.sendTipsCode(mahjong.TipCode_TCDonateFailedNoEnoughDiamond)
		return
	}
	...
	if len(r.players) > 0 {
		for _, p := range r.players {
			p.sendMsg(msgDonate, int32(mahjong.MessageCode_OPDonate))
		}
	}
}

// onDisbandRequest 处理玩家解散房间请求
func (r *Room) onDisbandRequest(user IUser, gmsg *mahjong.GameMessage) {
	r.cl.Printf("room disband applicate by user %s\n", user.userID())

	var stateConst = r.state.getStateConst()
	if stateConst != mahjong.RoomState_SRoomWaiting &&
		stateConst != mahjong.RoomState_SRoomPlaying {
		// 只有这两个状态下才允许解散房间
	...
	if r.handRoundStarted < 1 && r.ownerID != user.userID() {
		...
		r.sendDisbandError(user, mahjong.DisbandState_ErrorNeedOwnerWhenGameNotStart)
		return
	}
	...
	if r.disband != nil {
		...
		r.sendDisbandError(user, mahjong.DisbandState_ErrorDuplicateAcquire)
		return
	}
...
func (r *Room) sendDisbandError(user IUser, err mahjong.DisbandState) {
	...
	buf := formatGameMsg(msg, int32(mahjong.MessageCode_OPDisbandNotify))
	user.send(buf)
}

// onDisbandAnswer 处理其他玩家对解散房间请求的响应
func (r *Room) onDisbandAnswer(user IUser, gmsg *mahjong.GameMessage) {
	if r.disband == nil {
		r.cl.Println("room no in disband waiting state, discard disband answer")
		return
	}

	var msgDisbandAnswer = &mahjong.MsgDisbandAnswer{}
	err := proto.Unmarshal(gmsg.Data, msgDisbandAnswer)
	if err != nil {
		r.cl.Println("onDisbandAnswer unmarshal error:", err)
		return
	}
...
func (r *Room) appendHandScoreRecord(winType int) {
	...
	record := &mahjong.MsgRoomHandScoreRecord{}
	...
	if winType == int(mahjong.HandOverType_enumHandOverType_None) {
		// 如果是流局，则没有玩家得分列表
		return
	}

	playerRecords := make([]*mahjong.PlayerHandScoreRecord, len(r.players))
	for i, p := range r.players {
		playerRecord := &mahjong.PlayerHandScoreRecord{}
		var userID = p.userID()
...
func (r *Room) writePlayersStatis() {
	...
	for _, p := range r.players {
		...
		if p.sctx != nil {
			...
			// 牌型统计
			// add-date:2018-6-27
			if p.sctx.greatWinType != 0 {
				gwt := p.sctx.greatWinType
				for i := 0; i < 32; i++ {
					if (gwt & (1 << uint(i))) != 0 {
						gwtIdx := fmt.Sprintf("gwt%d", i)
						conn.Send("HINCRBY", dailyStatisTable, gwtIdx, 1)
					}
				}
			}
		}
	...
	for _, sr := range r.scoreRecords {
		for _, pr := range sr.PlayerRecords {
			if pr.GetUserID() == player.userID() {
				...
				case int32(mahjong.HandOverType_enumHandOverType_Chucker):
					player.gStatis.chuckerCounter++
					break
				case int32(mahjong.HandOverType_enumHandOverType_Win_SelfDrawn):
					player.gStatis.winSelfDrawnCounter++
					break
				case int32(mahjong.HandOverType_enumHandOverType_Win_Chuck):
					player.gStatis.winChuckCounter++
					break
				}
				break
			}
...
// takeOffDiamond 扣除钻石
func (r *Room) takeOffDiamond(userID string) (bool, mahjong.EnterRoomStatus) {
	r.cl.Println("takeOffDiamond, userID:", userID)
	if r.isForMonkey {
		return true, mahjong.EnterRoomStatus_Success
	}

	takeoffStatus := mahjong.EnterRoomStatus_TakeoffDiamondFailedNotEnough
	...
	if !succeed {
		...
		return false, mahjong.EnterRoomStatus_TakeoffDiamondFailedIO
	}
	...
	if status == int32(gconst.SSMsgError_ErrTakeoffDiamondFailedNotEnough) {
		...
		return false, mahjong.EnterRoomStatus_TakeoffDiamondFailedNotEnough
	}
...
func (r *Room) isContinuAble() bool {
	...
	return !r.isInLoseProtectedState()
}

// isInLoseProtectedState 是否足够的人进入失分保护状态
func (r *Room) isInLoseProtectedState() bool {
	protectedCount := 0
	if r.config.loseProtectEnabled {
		for _, p := range r.players {
			if p.gStatis.roundScore <= r.config.roomScoreConfig.loseProtectScore {
				protectedCount++
			}
		}
	}

	if r.config.playerNumAcquired == 4 && protectedCount >= 2 {
		return true
	}

	if r.config.playerNumAcquired < 4 && protectedCount >= 1 {
		return true
	}
...
// loseProtectTrimPay 保护状态下，应该付多少分, pay should great than 0
func (r *Room) loseProtectTrimPay(player *PlayerHolder, pay int) int {
	if r.config.loseProtectEnabled {
		var actualPay = pay
		if player.gStatis.roundScore > r.config.roomScoreConfig.loseProtectScore {
			if (player.gStatis.roundScore - pay) < r.config.roomScoreConfig.loseProtectScore {
				actualPay = player.gStatis.roundScore - r.config.roomScoreConfig.loseProtectScore
			}
		} else {
			actualPay = 0
		}

		r.cl.Printf("player %d in lose protect state, trim pay %d=>%d\n", player.chairID, pay, actualPay)
		return actualPay
	}
...
func (r *Room) updateUserLocation(userID string, location string) {
	...
	msg := &mahjong.MsgUpdateLocation{}
	msg.UserID = &userID
	msg.Location = &location
	buf := formatGameMsg(msg, int32(mahjong.MessageCode_OPUpdateLocation))
